 ...
 * > Hooks are awesome, the issue is not with hooks. **The issue is not hooks are bad**.
...
const useAccounts = function ({ userId }) {
  ...
  React.useEffect(() => {
    ...
    // one component, we need to make a consession because we may not receive a userId.
    //
...
function Root() {
  ...
  // 4. There are a lot of paths in this one component to think about wnen it comes to data.
